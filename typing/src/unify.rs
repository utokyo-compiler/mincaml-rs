...
pub fn unify<'ctx>(
    ...
    match (lhs.kind(), rhs.kind()) {
        (TyKind::TyVar(v), _) if let Some(ty) = subst.get(*v) => unify(dcx, subst, ty, rhs),
        (_, TyKind::TyVar(v)) if let Some(ty) = subst.get(*v) => unify(dcx, subst, lhs, ty),
        (TyKind::TyVar(v), _) => {
        ...
        (TyKind::Array(lhs_ty), TyKind::Array(rhs_ty)) => unify(dcx, subst, *lhs_ty, *rhs_ty),
