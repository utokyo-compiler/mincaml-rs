...
pub struct Diag<'dcx> {
    ...
    /// return value. In theory, return value optimization (RVO) should avoid
...
#[diagnostic::on_unimplemented(
    ...
    note = "consider using `Span`, `Option<Span>` or `AllowMissingSpan`"
)]
pub trait AddLabel {
    fn try_into_span(self) -> Option<Span>;
}

impl AddLabel for Span {
    fn try_into_span(self) -> Option<Span> {
        Some(self)
    }
}

impl AddLabel for Option<Span> {
    fn try_into_span(self) -> Option<Span> {
        self
    }
}
...
impl<'dcx> TryIntoMultiSpan<'dcx> for MultiSpan<'dcx> {
    fn try_into_spans(self) -> Option<MultiSpan<'dcx>> {
        Some(self)
    }
}
