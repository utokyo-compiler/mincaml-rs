...
thread_local! {
    pub(crate) static CODE_IDENT_COUNT: RefCell<u32> = const { RefCell::new(0) };
}
...
/// `Applicability` of a suggestion - mirrors `errors::Applicability` - and used to represent
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Applicability::MachineApplicable => {
                quote! { errors::Applicability::MachineApplicable }
            }
            Applicability::MaybeIncorrect => {
                quote! { errors::Applicability::MaybeIncorrect }
            }
            Applicability::HasPlaceholders => {
                quote! { errors::Applicability::HasPlaceholders }
            }
            Applicability::Unspecified => {
                quote! { errors::Applicability::Unspecified }
            }
        });
    }
}
...
fn parse_suggestion_values(nested: ParseNestedMeta<'_>) -> syn::Result<Vec<LitStr>> {
...
pub(super) fn build_suggestion_code(
    ...
    let values = match parse_suggestion_values(nested) {
        Ok(x) => x,
    ...
    pub(crate) fn to_suggestion_style(self) -> TokenStream {
        match self {
            SuggestionKind::Normal => {
                quote! { errors::SuggestionStyle::ShowCode }
            }
            SuggestionKind::Short => {
                quote! { errors::SuggestionStyle::HideCodeInline }
            }
            SuggestionKind::Hidden => {
                quote! { errors::SuggestionStyle::HideCodeAlways }
            }
            SuggestionKind::Verbose => {
                quote! { errors::SuggestionStyle::ShowAlways }
            }
            SuggestionKind::ToolOnly => {
                quote! { errors::SuggestionStyle::CompletelyHidden }
            }
        }
    }
...
impl SubdiagnosticVariant {
    ...
    pub(super) fn from_attr(
        attr: &Attribute,
        fields: &impl HasFieldMap,
    ) -> Result<Option<SubdiagnosticVariant>, DeriveError> {
        ...
        list.parse_nested_meta(|nested| {
            if nested.input.is_empty() || nested.input.peek(Token![,]) {
                if first {
                    ...
                    span_err(nested.input.span(), "a diagnostic slug must be the first argument to the attribute").emit();
                }
            ...
            let path_span = nested.path.span();
            let val_span = nested.input.span();
            ...
            match (nested_name, &mut kind) {
                ...
                (
                    ...
                    let value = Applicability::from_str(&value.value()).unwrap_or_else(|()| {
                        span_err(value.span(), "invalid applicability").emit();
                        has_errors = true;
                        Applicability::Unspecified
                    });
                ...
                (
                    ...
                    let value = value.value().parse().unwrap_or_else(|()| {
                        span_err(value.span(), "invalid suggestion style")
