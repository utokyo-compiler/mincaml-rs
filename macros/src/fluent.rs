...
/// Final tokens.
fn finalize(body: &TokenStream, resource: &TokenStream) -> proc_macro::TokenStream {
    quote! {
        /// Raw content of Fluent resource for this crate, generated by `fluent_messages` macro,
        /// imported by `driver` to include all crates' resources in one bundle.
        ...
        pub(crate) mod fluent_generated {
            ...
            pub mod _subdiag {
                /// Default for `#[help]`
                pub const help: errors::FluentAttr =
                    errors::FluentAttr(std::borrow::Cow::Borrowed("help"));
                /// Default for `#[note]`
                pub const note: errors::FluentAttr =
                    errors::FluentAttr(std::borrow::Cow::Borrowed("note"));
                /// Default for `#[warn]`
                pub const warn: errors::FluentAttr =
                    errors::FluentAttr(std::borrow::Cow::Borrowed("warn"));
                /// Default for `#[label]`
                pub const label: errors::FluentAttr =
                    errors::FluentAttr(std::borrow::Cow::Borrowed("label"));
                /// Default for `#[suggestion]`
                pub const suggestion: errors::FluentAttr =
                    errors::FluentAttr(std::borrow::Cow::Borrowed("suggestion"));
            }
        }
    }
    .into()
...
fn failed(crate_name: &Ident) -> proc_macro::TokenStream {
    finalize(&quote! { pub mod #crate_name {} }, &quote! { "" })
}

/// See [`macros::fluent_messages`].
pub(crate) fn fluent_messages(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let mut previous_idents = HashSet::new();
            ...
            } in errs
            {
                ...
                let message = annotate_snippets::Level::Error.title(&err).snippet(
                    Snippet::source(this.source())
                        ...
                        .fold(true)
                        .annotation(annotate_snippets::Level::Error.span(pos.start..pos.end)),
                );
    ...
    for entry in resource.entries() {
        if let Entry::Message(msg) = entry {
            ...
            if !previous_idents.insert(snake_name.clone()) {
                continue;
            }
            ...
            constants.extend(quote! {
                #[doc = #docstr]
                pub const #snake_name: errors::FluentIdentifier =
                    errors::FluentIdentifier(std::borrow::Cow::Borrowed(#name));
            });
            ...
            {
                let snake_name = Ident::new(&attr_name.replace('-', "_"), resource_str.span());
                if !previous_idents.insert(snake_name.clone()) {
                    continue;
                }
                ...
                let msg = format!("Constant referring to Fluent attribute `.{attr_name}`");
                constants.extend(quote! {
                    #[doc = #msg]
                    pub const #snake_name: errors::FluentAttr =
                        errors::FluentAttr(std::borrow::Cow::Borrowed(#attr_name));
                });
            }
    ...
    finalize(&constants, &quote! { include_str!(#relative_ftl_path) })
}
